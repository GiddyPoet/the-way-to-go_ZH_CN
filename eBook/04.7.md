...
`TrimRight`和`TrimLeft`只会删除子串中所有包含的**字符**，如果需要删除指定的**字符串**可以使用`TrimSuffix`和`TrimPreffix`。
...
func main() {
	...
	result = strings.TrimSuffix("Hello Gophers", " Gophers")
	fmt.Println(result, len(result))
...
`strings.SplitAfter`分割后保留分隔符。

`strings.SplitAfterN`指定切割次数，同时也保留分隔符。
...
func main() {
	...
	for _, v := range strings.SplitAfter(str, "_") {
		fmt.Println(v)
	}
}
```
...
`Join` 用于将元素类型为 string 的 slice 使用分割符号来拼接组成一个字符串，在已有字符串的时候用`strings.Join`需要在字符串中添加分隔符，用`Strings.Join`性能比较好：
...
* 如果需要拼接的不仅仅是字符串，还有数字之类的其他需求的话，可以考虑 fmt.Sprint
