...
> `type-switch`不仅可以用来判断结构体类型，也能判断接口类型，同时也可以判断是否包含该方法
...
> 获取最小值的方法，核心就在于对切片内容进行单次最小排序，其中最小的会放在切片末尾
...
func (a IntA) Elem(index int) interface{} {
	return a[index]
}
