...
> 实际上通过reflect判断当前结构体是否实现了可导出的函数
> 因此，如果采用指针定义的函数，则原型没有拥有可导出的函数
> 而指针则可以隐式的转换成原型
...
func main() {
	conn, err := net.Dial("tcp", "localhost:7353")
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	client.Call("Arith.Multi", ArithReq{N: 7, M: 4}, resp)

	fmt.Println(resp.Rem, resp.Div, resp.Multi)
}
```
> sever tcp
```golang
...
func (a *Arith) Multi(req ArithReq, res *ArithResp) error {
	res.Multi = req.M * req.N
	res.Div = req.N / req.M
	res.Div = req.N % req.M
	return nil
}
...
func main() {
	...
	listener, err := net.Listen("tcp", "localhost:7353")
	if err != nil {
		fmt.Println(err)
		return
	}
